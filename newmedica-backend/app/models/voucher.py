...
from .user_type import UserType  # Ensure table registration before FK resolution
from .user import User  # Ensure table registration before FK resolution
...
class Voucher(SQLModel, table=True):
    ...
    target_user_type_id: Optional[uuid.UUID] = Field(default=None, foreign_key="usertype.id")
    target_user_id: Optional[uuid.UUID] = Field(default=None, foreign_key="user.id")

    # rules
    min_quantity: int = Field(default=0, description="Minimum quantity of matched products to activate")
    per_unit: bool = Field(default=False, description="Apply amount per matched unit instead of per order")
    ...
    # Product relationships are defined on Product to avoid circular imports.
