...
@pytest.mark.asyncio
async def test_create_product(async_client: AsyncClient, admin_token_headers: dict, session: AsyncSession):
...
async def test_get_products(async_client: AsyncClient, session: AsyncSession):
    ...
    product = Product(name="Product 1 for Get", description="...", price=10.0, stock=10, category_id=category.id)
    session.add(product)
    await session.commit()
...
@pytest.mark.asyncio
async def test_upload_media_for_product(async_client: AsyncClient, admin_token_headers: dict, session: AsyncSession):
    category = Category(name="Media Test Cat", description="Desc")
    session.add(category)
    await session.commit()
    await session.refresh(category)

    product = Product(name="Media Product", description="...", price=1.0, stock=1, category_id=category.id)
    session.add(product)
    await session.commit()
    ...
    response = await async_client.post(f"/api/v1/products/{product.id}/media", files=files, data=data, headers=admin_token_headers)
    assert response.status_code == 201
...
async def test_update_media_order(async_client: AsyncClient, admin_token_headers: dict, session: AsyncSession):
    ...
    product = Product(name="Media Order Product", description="...", price=1.0, stock=1, category_id=category.id)
    media1 = ProductMedia(product=product, media_type="image", url="/url1", display_order=1)
    media2 = ProductMedia(product=product, media_type="image", url="/url2", display_order=2)
    ...
    response = await async_client.put(f"/api/v1/products/{product.id}/media/order", json={"media_ids": new_order}, headers=admin_token_headers)
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_delete_media(async_client: AsyncClient, admin_token_headers: dict, session: AsyncSession):
    category = Category(name="Media Delete Cat", description="Desc")
    product = Product(name="Media Delete Product", description="...", price=1.0, stock=1, category_id=category.id)
    media = ProductMedia(product=product, media_type="image", url="/todelete", display_order=1)
    session.add_all([category, product, media])
    await session.commit()
    await session.refresh(media)

    response = await async_client.delete(f"/api/v1/media/{media.id}", headers=admin_token_headers)
...
async def test_get_product_by_id(async_client: AsyncClient, session: AsyncSession):
    ...
    product = Product(name="Get By ID Product", description="...", price=1.0, stock=1, category_id=category.id)
...
@pytest.mark.asyncio
async def test_update_product(async_client: AsyncClient, admin_token_headers: dict, session: AsyncSession):
    category = Category(name="Update Cat", description="Desc")
    product = Product(name="Product to Update", description="Old desc", price=10.0, stock=10, category_id=category.id)
    session.add_all([category, product])
    await session.commit()
    await session.refresh(product)
...
@pytest.mark.asyncio
async def test_delete_product(async_client: AsyncClient, admin_token_headers: dict, session: AsyncSession):
    category = Category(name="Delete Cat", description="Desc")
    product = Product(name="Product to Delete", description="...", price=1.0, stock=1, category_id=category.id)
    session.add_all([category, product])
    await session.commit()
    await session.refresh(product)

    response = await async_client.delete(f"/api/v1/products/{product.id}", headers=admin_token_headers)
    assert response.status_code == 204
...
@pytest.mark.asyncio
async def test_get_products_filtered_by_category(async_client: AsyncClient, session: AsyncSession):
    cat1 = Category(name="Filter Cat 1", description="Desc 1")
    cat2 = Category(name="Filter Cat 2", description="Desc 2")
    prod1 = Product(name="Filter Prod 1", description="A product for filtering", price=10, stock=1, category=cat1)
    prod2 = Product(name="Filter Prod 2", description="Another product for filtering", price=20, stock=1, category=cat2)
...
@pytest.mark.asyncio
async def test_get_products_sorted_by_price(async_client: AsyncClient, session: AsyncSession):
    category = Category(name="Sort Cat", description="Desc")
    prod1 = Product(name="Sort Prod 1", description="A product for sorting", price=20.0, stock=1, category=category)
    prod2 = Product(name="Sort Prod 2", description="Another product for sorting", price=10.0, stock=1, category=category)
